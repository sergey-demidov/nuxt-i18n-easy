...
lib.writeConfig = (locale, sentences) => {
  ...
  Object.keys(sentences).forEach((key, index) => {
    let line = '  '
    sentences[key].translated = sentences[key].translated.replace(/'/g, "\\'")
...
lib.getConfig = (nuxtConfig, config) => {
  ...
  if (Array.isArray(i18n.locales) && i18n.locales.length > 0) {
    config.locales = lib.clone(i18n.locales)
  }
...
lib.importFile = (fileName) => {
  try {
    const entry = fs.readFileSync(path.join(process.cwd(), fileName), 'utf-8')
    const res = babel.transformSync(entry, {
      cloneInputAst: true,
      babelrc: false,
      configFile: false,
      passPerPreset: false,
...
lib.translate = async (from, to, sentence) => {
  ...
  try {
    ...
    return { translated: sentence, opts: ['Error: cant fetch google translate API'] }
  }
