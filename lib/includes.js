...
lib.processSentences = async (phrases, sourceLanguage, locale) => {
  ...
  Object.keys(translations).forEach((key) => {
    ...
    res.stringNumber = typeof stringNumbers[key] === 'string' ? ' // ' + stringNumbers[key] : ''
    result[key] = res
  })
  return result
}
...
lib.writeConfig = (locale, sentences) => {
  ...
  Object.keys(sentences).forEach((key, index) => {
    ...
    line += sentences[key].stringNumber
    line += sentences[key].opts
  ...
  if (fs.statSync(locale.langFile).size > 0) {
