...
lib.processSentences = async (phrases, sourceLanguage, locale) => {
  ...
  for (const key of diffs) {
    ...
    if (res.lineNumber) {
      lineNumbers[key] = res.lineNumber
    }
  }
  Object.keys(translations).forEach((key) => {
    ...
    res.lineNumber = typeof lineNumbers[key] === 'number' ? lineNumbers[key] : ''
    result[key] = res
  })
  return result
}
...
lib.writeConfig = (locale, sentences) => {
  ...
  Object.keys(sentences).forEach((key, index) => {
    ...
    if (sentences[key].lineNumber || sentences[key].opts || sentences[key].unused) {
      line += ' // '
      if (sentences[key].lineNumber) { line += sentences[key].lineNumber + ' ' }
